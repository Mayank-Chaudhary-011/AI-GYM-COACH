...
from services.vision.exercise_vedio_processor import VideoProcessorClass
    ...
    if "voice_pipeline" not in st.session_state:
        try:
            ...
            if api_key:
                ...
                st.warning("GROQ_API_KEY not found; using local voice coaching fallback.")
            ...
            st.session_state.voice_pipeline = VoicePipeline(LLMCoach(None), TextToSpeech())
    ...
    with st.sidebar:
        ...
        if st.session_state.get("voice_pipeline") is None:
            ...
            if st.session_state.get("voice_error"):
                st.info(f"Voice init error: {st.session_state.voice_error}")
